...
const SavedBtnComp = ({ data }) => {
  ...
  return (
    <button
      className="outline-0 border-0 bg-none curser-pointer"
      ...
      "
          >
            <tr>
              ...
              <th className="py-1">7D Change</th>
            </tr>
          </thead>
          <tbody>
            {cryptoData.map((dt) => {
          ...
          "
                >
                  ...
                  <td className="py-4">{dt.name}</td>
                  <td className="py-4">{dt.current_price}</td>
                  <td className="py-4">
                    {dt.market_cap_change_percentage_24h}
                  </td>
                  <td
                    className={
                      dt.price_change_percentage_24h_in_currency > 0
                        ? "text-green py-4"
                        : "text-red py-4"
                    }
                  >
                    {Number(dt.price_change_percentage_24h_in_currency).toFixed(
                      2
                    )}
                  </td>
                  <td
                    className={
                      dt.price_change_percentage_7d_in_currency > 0
                        ? "text-green py-4"
                        : "text-red py-4"
                    }
                  >
                    {Number(dt.price_change_percentage_7d_in_currency).toFixed(
                      2
                    )}
                  </td>
                </tr>
